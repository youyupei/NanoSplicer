...
################################## mean denoising
            ...
            if True and j == num_of_cand - 1:
                ...
                distinguish_points_idx = \
                    np.where(np.abs(squiggle_match[0][:,0] - squiggle_match[1][:,0]) > \
                     squiggle_match[1][:,1])[0]
                junction_squiggle_mean = mean_denoise(junction_squiggle, [squiggle_match[0], squiggle_match[1]])
                
                # truncated
                LR_con = __log_likelihood(junction_squiggle_mean, 
                            squiggle_match[0][:,0],squiggle_match[0][:,1]) - \
                        __log_likelihood(junction_squiggle_mean, 
                            squiggle_match[1][:,0],squiggle_match[1][:,1])
                fig, axes = plt.subplots(nrows=2, figsize=(20,12))

                unique_LR, counts = np.unique(LR_con, return_counts=True)
                unique_LR_dst_p, counts_dst_p = np.unique(LR_con[distinguish_points_idx],
                                                                         return_counts=True)
                ...
                axes[0].plot(squiggle_match[1][:,0], 'g',linewidth=0.4,
                        label = "NanoSplicer candidate")
                axes[0].plot(junction_squiggle_mean, 'b',linewidth=0.4, 
                        label = "junction squiggle")
                axes[0].legend(frameon=False, loc='best')
                axes[0].set_title(
                    "LR_full (all segments) = {:.2f}, # of distinguish segments = {}, LR_middle = {:.2f}(count >= 4), adj_dist_full = {:.2f},{:.2f}, adj_dist_middle = {:.2f},{:.2f}\n {}\n{}".format(
                    sum(unique_LR), len(unique_LR_dst_p),
                    sum(unique_LR_dst_p[counts_dst_p>=4]),
                    #np.exp(sum(set(list(LR_con[distinguish_points_idx]))))/(1 + np.exp(sum(set(list(LR_con[distinguish_points_idx]))))),
                    -score_dict[0]/len(path),-score_dict[1]/len(path),
                    np.mean(np.append(cum_path[0][0], cum_path[0][1:] - cum_path[0][:-1])[distinguish_points_idx]),
                    np.mean(np.append(cum_path[1][0], cum_path[1][1:] - cum_path[1][:-1])[distinguish_points_idx]),
                    aligned_base[0], aligned_base[1]
...
#################################### median denoising
            ...
            if False and j == num_of_cand - 1:
                ...
                distinguish_points_idx = \
                    np.where(np.abs(squiggle_match[0][:,0] - squiggle_match[1][:,0]) > \
                     squiggle_match[1][:,1])[0]
                junction_squiggle_median = median_denoise(junction_squiggle, [squiggle_match[0],squiggle_match[1]] )

                LR_con = laplac_log_likelihood(junction_squiggle_median, squiggle_match[0]) - laplac_log_likelihood(junction_squiggle_median, squiggle_match[1])
                fig, axes = plt.subplots(nrows=2, figsize=(20,12))
                ...
                axes[0].plot(squiggle_match[1][:,0], 'g',linewidth=0.4,
                        label = "NanoSplicer candidate")
                axes[0].plot(junction_squiggle_median, 'b',linewidth=0.4, 
                        label = "junction squiggle")
                axes[0].legend(frameon=False, loc='best')
                axes[0].set_title(
                    "LR_full = {:.2f}, # of distinguish points = {}, LR_middle = {:.2f}, adj_dist_full = {:.2f},{:.2f}, adj_dist_middle = {:.2f},{:.2f}\n {}\n{}".format(
                ...
                if True:
                    ...
                    ax2.set_ylabel('log likelihood ratio contribution', color='#4b0082')
                    ax1.legend(frameon=False, loc='best')
                    fig.savefig("{}_median.png".format(read.qname))
                    plt.close()
